...
HTML;
	...
	private function thread_template($thread){
		$class           = apply_filters('fe_thread_item_class', 'thread-item ', $thread );
		...
		$fe_status = apply_filters( 'forumengine_filter_thread_status', '', $thread );
		$is_sticky = et_is_sticky_thread($thread->ID);
		$is_sticky_home = et_is_sticky_thread($thread->ID, true);

		$template = <<<HTML
<li class="{$isHightLight} {$class}" data-id="{$thread->ID}" data-cat="{$thread->thread_category[0]->slug}">
	{$fe_status}
	{$avatar_thumb}
